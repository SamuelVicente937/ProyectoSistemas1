...
const FormLink = ({ onSubmit, isSubmitting = false }: FormLinkProps) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
      ...
      {selectedHorario && selectedMateria && (
        <div className="bg-red-50 border-2 border-[#a00000] rounded-lg p-4 col-span-full">
          <h3 className="font-bold text-[#a00000] mb-3">
            Información del Formulario a generar:
          </h3>
          <ul className="space-y-2 text-sm  text-[#767676]">
            ...
            <li>
              ...
              {formData.id_lab
                ? laboratorios.find(
                    (l) => l.id_lab === parseInt(formData.id_lab)
                  )?.codigo_lab || "No seleccionado"
                : "No seleccionado"}
            </li>
          </ul>
        </div>
